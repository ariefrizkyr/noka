...
export const updateUserSettingsSchema = z.object({
  ...
  onboarding_step_3_completed: z.boolean().optional(),
  onboarding_current_step: z.number().int().min(1).max(3).optional(),
});
...
export const updateAccountSchema = z.object({
  ...
  type: z
    .enum(["bank_account", "credit_card", "investment_account"] as const)
    .optional(),
...
export const createTransactionSchema = z
  .object({
    ...
    transaction_date: z
      .string()
      .datetime()
      .or(z.string().regex(/^\d{4}-\d{2}-\d{2}$/)), // ISO date or datetime
  ...
  .refine(
    ...
    {
      message:
        "Transfer requires both from_account_id and to_account_id. Income/expense requires account_id.",
    },
  );
