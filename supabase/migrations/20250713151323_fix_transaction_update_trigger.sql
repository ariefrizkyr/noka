-- Fix transaction UPDATE trigger to properly handle balance updates
-- This migration updates the trigger function to handle UPDATE operations
...
-- Drop existing triggers and recreate with UPDATE support
...
-- Create comprehensive trigger to handle INSERT, UPDATE and DELETE operations
CREATE TRIGGER trigger_update_account_balance
AFTER INSERT OR UPDATE OR DELETE ON transactions
FOR EACH ROW EXECUTE FUNCTION update_account_balance_with_ledger();

-- Add a comment to document the change
COMMENT ON FUNCTION update_account_balance_with_ledger() IS 'Handles account balance updates for transaction INSERT, UPDATE, and DELETE operations. UPDATE operations reverse old transaction effects and apply new ones.';
COMMENT ON TRIGGER trigger_update_account_balance ON transactions IS 'Comprehensive trigger that handles balance updates for all transaction operations (INSERT, UPDATE, DELETE) with proper balance reversal logic';