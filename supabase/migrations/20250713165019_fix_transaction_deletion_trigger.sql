-- Fix transaction deletion trigger to properly handle balance updates
-- This migration updates the trigger function to handle DELETE operations
-- instead of throwing an exception

-- Drop the existing trigger first
DROP TRIGGER IF EXISTS trigger_update_account_balance ON transactions;

-- Update the trigger function to handle DELETE operations
...
-- Recreate the trigger to handle INSERT and DELETE operations
CREATE TRIGGER trigger_update_account_balance
AFTER INSERT OR DELETE ON transactions
FOR EACH ROW EXECUTE FUNCTION update_account_balance_with_ledger();

-- Add a comment to document the change
COMMENT ON FUNCTION update_account_balance_with_ledger() IS 'Handles account balance updates for transaction INSERT and DELETE operations. UPDATE operations are intentionally not supported and should be handled at application level.';