-- Fix GROUP BY clause in get_budget_progress function
-- The issue is using ac.* but not including all columns in GROUP BY clause
-- Specifically, user_id from categories is selected via ac.* but not grouped by
...
BEGIN
    ...
    accessible_categories AS (
        -- Personal categories - select only the columns we need
        SELECT c.id, c.name, c.type, c.icon, c.budget_amount, c.budget_frequency, 
               c.is_shared, FALSE as is_family, NULL::UUID as cat_family_id, NULL::VARCHAR as cat_family_name,
        ...
        -- Shared family categories - select only the columns we need
        SELECT c.id, c.name, c.type, c.icon, c.budget_amount, c.budget_frequency,
               c.is_shared, TRUE as is_family, c.family_id as cat_family_id, uf.family_name as cat_family_name,
               v_period_start as period_start, v_period_end as period_end
        FROM public.categories c
    ...
    category_spending AS (
        SELECT 
            ac.id, ac.name, ac.type, ac.icon, ac.budget_amount, ac.budget_frequency,
            ac.is_shared, ac.is_family, ac.cat_family_id, ac.cat_family_name,
            ac.period_start, ac.period_end,
    ...
    budget_calculations AS (
        ...
        GROUP BY cs.id, cs.name, cs.type, cs.icon, cs.budget_amount, cs.budget_frequency, 
                 cs.is_shared, cs.is_family, cs.cat_family_id, cs.cat_family_name,
                 cs.period_start, cs.period_end, cs.total_spent
    )
    SELECT 
        bc.id,
        bc.name,
...
-- Add comment
COMMENT ON FUNCTION get_budget_progress(UUID) IS 'Family Sharing: Fixed GROUP BY clause by explicitly selecting required columns instead of using wildcard';